...
def traitement_parcours(path_parcours, params,hote, port_sender, sender_udp):
    ...
    try:
        ...
        msg_udp = "ErreurMoteur: impossible de charger le fichier de parcours " + str(file_parcours) + "#"
        print msg_udp
        logging.error(msg_udp, exc_info=True)
    ...
    try:
        ...
        for t in range(0, LAT_corde_2.size):
            x = lat_to_m(LAT_corde[t] - centre_lat, centre_lat)
            y = lon_to_m(LONG_corde[t] - centre_long, centre_lat)
            X_corde.append(x)
            Y_corde.append(y)
            ...
            X_corde_2.append(x)
            Y_corde_2.append(y)
            x = lat_to_m(LAT_corde_ext[t] - centre_lat, centre_lat)
            y = lon_to_m(LONG_corde_ext[t] - centre_long, centre_lat)
            X_corde_ext.append(x)
            Y_corde_ext.append(y)
        ...
        for p in range(0, np.size(PC)):
            if not PC[p] == '0':
                ...
                if PC[p] == 'PCARR':
                    ind_fin = p
                    indice_portes[-1] = np.size(corde_interpol_x) - 1
        ...
        msg_udp = "ErreurMoteur: Le parcours n'a pas pu etre traite correctement#"
        print msg_udp
        logging.error(msg_udp, exc_info=True)
...
########### OLD ##############
# def ts_to_dur(timestamp):
#     """"
# 	calcul une duree en secondes à partir d'un timestamp au format hhmmss.mmmm
#     :param timestamp:
#     :return dur: duree en millisecondes
# 	"""
#     try:
#         heure = np.floor(float(timestamp) / float(10 ** 4))
#         minute = np.floor((timestamp - heure * 10 ** 4) / float(10 ** 2))
#         sec = timestamp - heure * 10 ** 4 - minute * 10 ** 2
#         dur = heure * 60 * 60 + minute * 60 + sec
#         return dur * 1000
#     except:
#         logging.error("erreur lors de la conversion du timestamp",exc_info=True)
#         return 0
...
def init_depart(Mes, Ind, par, nb_handicaps, id_porte_dep, k, top_depart):
            ...
            if STATE_UDP:
                ...
                try:
                    ...
                    while t_analyse < ts_fin and STATE_UDP and connected:
                        if num_analyse % 5 == 0:
                            ...
                            for k in range(0, nb_id):
                                if Mes.lat_mesure[k] != 0 and Mes.long_mesure[k] != 0: # and Ind.donnee_precise[k]:
                                    plt.plot(Mes.lat_mesure[k],Mes.long_mesure[k],'.',label = str(k))
                                    plt.plot(Mes.lat_mesure[k], Mes.long_mesure[k], '.', label=str(k))
                            ind1 = np.maximum(0,float(np.max(Mes.ind_parcours_mesure))/ float(par.res_interp) - 10.0 )
                            ind2 = np.minimum(float(np.max(Mes.ind_parcours_mesure))/ float(par.res_interp) + 10.0 , np.size(lat_corde)-1)
                        ...
                        top_depart = 0
                        if np.sum(Mes.depart_franchis) > np.int(nb_id / 4) and attente_depart:
                            DEPART_COURSE = 1
                            attente_depart = 0
                        ...
                        # print(msg_recu)
                            ...
                            msg_udp = "ErreurMoteur: pas de trame recue" + str(k) + "#"
                            print msg_udp
                            logging.error(msg_udp, exc_info=True)
                        ...
                        if np.sum(Ind.arrive) == np.sum(mode_course) and not tous_arrives: #and COURSE_TERMINEE
                            Ind = Indicateurs(nb_id, par)
                            Mes = Mesures(nb_id,par)
                            mode_course = np.zeros(nb_id)
                            COURSE_DEMARREE = 0
                            tous_arrives = 1
                        ...
                        # Si le top dapert vient d'être donne,
                        ...
                        if top_depart:
                            TS_DEPART = np.max(Mes.ts_mesure)
                            liste_ts_dep = [TS_DEPART]
                            for i in range(0, nb_id):
                                if (Mes.depart_franchis[i] and TS_DEPART - Mes.ts_passage_dep[i] > 20000) or not Mes.depart_franchis[i]:
                                    Mes.depart_franchis[i] = 1
                                    Mes.ts_passage_dep[i] = TS_DEPART
                                    Mes, Ind = init_depart(Mes, Ind, par, nb_handicaps, id_porte_dep, i, top_depart)
                                    print "prise en compte top depart ", str(i), Ind.ind_porte_courante[i], id_portes[int(Ind.ind_porte_courante[i])]

                                    print Ind.ind_porte_courante[i], np.size(indice_portes), Mes.depart_franchis[
                                        i], Ind.arrive[i], Mes.arr_franchis[i], Mes.ind_parcours_mesure[i] , indice_portes[int(Ind.ind_porte_courante[i])], \
                                            id_portes[int(Ind.ind_porte_courante[i])],Ind.en_course[i]
                                    """
                                    if (Ind.ind_porte_courante[k] < np.size(indice_portes) and Mes.depart_franchis[
                                        ...
                                        if Mes.ind_parcours_mesure[k] >= indice_portes[int(Ind.ind_porte_courante[k])] or Mes.arr_franchis[k]:
                                    ...
                                    liste_ts_dep = np.append(liste_ts_dep, Mes.ts_passage_dep[i])
                            TS_DEPART = np.min(liste_ts_dep)
                        ...
                        for k in range(0, nb_id):
                            ...
                            if trame_recue:  # si on a recu une trame pour ce capteur
                                ...
                                # Conversion des donnees lat / long
                                #trame[2] = trame[2] ### 1205 conv_deg_minutes(trame[2])
                                #trame[3] = trame[3] ### 1205 conv_deg_minutes(trame[3])
                            ...
                            if calculer_position and MODE == "course" and not (Mes.arr_franchis[
                                                                                    k] or tous_arrives):  # ajout not tous_arrives 0706 # and not Mes.arr_franchis[k]:

                                # On vérifie si on a franchis la ligne d'arrivée
                                Mes.arr_franchis[k] = passage_porte(Mes.Obs[0], Mes.Obs[1], Mes.prev_x_mesure[k],
                                                                    Mes.prev_y_mesure[k], porte_arr) and int(
                                    Ind.ind_porte_courante[k]) >= np.size(id_portes) - 1

                                # Si l'arrivee est franchie, on met fin au parcours
                                if Mes.arr_franchis[k] and not Ind.arrive[k]:
                                    Mes.ind_parcours_mesure[k] = np.size(corde_interpol_x) + 1
                                    Mes.depart_franchis[k] = 0
                                    msg_udp = "EvtMoteur: arrive " + str(k) + " " + str(num_analyse) + "#"
                                    print msg_udp
                                    logging.info(msg_udp)
                                ...
                                if Mes.depart_franchis[k] and not COURSE_DEMARREE:
                                    if Mes.ts_mesure[k] - Mes.ts_passage_dep[k] > 20000:
                                        Mes, Ind = init_depart(Mes, Ind, par, nb_handicaps, id_porte_dep, k, top_depart)
                                        Mes.depart_franchis[k] = 0
                                        Mes.ind_parcours_mesure[k] = -1
                                ...
                                if Mes.depart_franchis[k] and not Ind.arrive[k]: # (Mes.arr_franchis[k] or ):
                                    if trame[2] != 0 and trame[3] != 0 and trame[1]!= 0 :
                                        if (trame[4] == 0.0 and Mes.v_mesure[k] > 3.0) or (trame[4] > 25.0) \
                                                or (abs(Mes.v_mesure[k] - trame[4]) > 3.0 and Mes.v_mesure[
                                                    k] > 3.0):
                                        ...
                                        if mindist > diagonale + 10 or mindist_ext > diagonale + 10:
                                            Mes.Obs = [0, 0, 0]
                                            #print "prediction 3 " + str(k)
                                            # print mindist, mindist_ext, diagonale + 5, Mes.ts_mesure[k
                                            # ], str(k)

                                    # On vérifie si le départ a été franchi
                                    depart_franchis = passage_porte(Mes.x_mesure[k], Mes.y_mesure[k],
                                                                    Mes.prev_x_mesure[k],
                                                                    Mes.prev_y_mesure[k],
                                                                    portes_depart_handicap[int(id_porte_dep[k])])

                                    #### Si le départ est franchis, alors qu'il avait déjà été franchis et qu'il n'y a
                                    # pas encore eu le top départ
                                    if depart_franchis and not (
                                        (Mes.depart_franchis[k] and COURSE_DEMARREE) or Mes.arr_franchis[k]):
                                        # On initialise
                                        Mes, Ind = init_depart(Mes, Ind, par, nb_handicaps, id_porte_dep, k, top_depart)
                                        # On met à jour le timestamp de départ
                                        ...
                                        print "nouveau depart ", str(i), Ind.ind_porte_courante[k], \
                                    ...
                                    dD = Mes.Etat[2] * par.dt
                                    avance = 0
                                    ind_avance = np.int(Mes.stock_proche_corde[k])
                                    dx = 0
                                    dy = 0
                                    while avance < dD and ind_avance < np.size(corde_interpol_x):
                                        ...
                                        dx = nouveau_x - Mes.Etat[0]
                                        dy = nouveau_y - Mes.Etat[1]
                                    ...
                                    ind_parcours = np.int(Mes.stock_proche_corde[k])
                                    dist_x = corde_interpol_x[ind_parcours] - Mes.Etat[0]
                                    dist_y = corde_interpol_y[ind_parcours] - Mes.Etat[1]
                                    distance_corde = np.sqrt(np.square(dist_x) + np.square(dist_y))
                                    prev_distance_corde = distance_corde
                                    d_avance = 0
                                    loops = 0
                                    recherche_en_cours = True
                                    while distance_corde <= prev_distance_corde and recherche_en_cours:
                                        ind_parcours = ind_parcours + 1
                                        if ind_parcours >= np.size(corde_interpol_x):
                                            recherche_en_cours = False
                                            Mes.arr_franchis[k] = 1
                                        prev_distance_corde = distance_corde
                                        dist_x = corde_interpol_x[ind_parcours - 1] - Mes.Etat[0]
                                        dist_y = corde_interpol_y[ind_parcours - 1] - Mes.Etat[1]
                                        distance_corde = np.sqrt(np.square(dist_x) + np.square(dist_y))
                                        loops = loops + 1
                                        # d_avance = np.float(d_avance) + np.float(10) / np.float(par.res_interp)
                                        # UPDATE 0305: avance sur la corde 2
                                        d_avance = np.float(d_avance) + dD_corde_2[ind_parcours - 1]
                                        # np.float(10) / np.float(par.res_interp)

                                    ind_parcours = np.minimum(ind_parcours, np.size(corde_interpol_x))

                                    Mes.stock_proche_corde[k] = ind_parcours - 1
                                    Mes.stock_distance_corde[k] = prev_distance_corde
                                    distance_corde = prev_distance_corde

                                    #### Sortie mesure
                                    Mes.lat_mesure[k] = m_to_lat(Mes.Etat[0], centre_lat) + centre_lat
                                    Mes.long_mesure[k] = m_to_lon(Mes.Etat[1], centre_lat) + centre_long
                                    # Mes.lat_mesure[k] = trame[2] # m_to_lat(Mes.Etat[0], centre_lat) + centre_lat
                                    # Mes.long_mesure[k] = trame[3] # m_to_lon(Mes.Etat[1], centre_lat) + centre_long

                                    Mes.avance_mesure[k] = Mes.avance_mesure[k] + d_avance# - dD_corde_2[ind_parcours]
                                    ...
                                    Mes.precision_mesure[k] = np.float(np.sqrt(np.float(P[1, 1])+ np.float(P[0, 0])))
                                    ...
                                    if trame[2] != 0 and trame[3] != 0:
                                        ...
                                        depart_franchis = passage_porte(Mes.x_mesure[k], Mes.y_mesure[k],
                                                                        Mes.prev_x_mesure[k], Mes.prev_y_mesure[k],
                                                                        portes_depart_handicap[int(id_porte_dep[k])])

                                        Mes.ind_parcours_mesure[k] = -1
                                        if  depart_franchis and not Ind.arrive[k]:

                                            if not COURSE_DEMARREE or (COURSE_DEMARREE and (Mes.ts_mesure[k] - TS_DEPART) < 10000 ):
                                        ...
                                        if Mes.depart_franchis[k]:
                                            Mes, Ind = init_depart(Mes, Ind, par, nb_handicaps, id_porte_dep, k, top_depart)
                                            print "depart ", str(i), Ind.ind_porte_courante[k], \
                                            id_portes[int(Ind.ind_porte_courante[k])]
                                            msg_udp = "EvtMoteur: depart franchis " + str(k) + "#"
                                            print msg_udp
                                            logging.info(msg_udp)
                                    ...
                                    if (Ind.ind_porte_courante[k] < np.size(indice_portes) and Mes.depart_franchis[
                                        ...
                                        if Mes.ind_parcours_mesure[k] >= indice_portes[int(Ind.ind_porte_courante[k])] or Mes.arr_franchis[k]:
                                            ...
                                            if id_portes[int(Ind.ind_porte_courante[k])].startswith('PCDEP') and not Ind.en_course[k]:
                                                ...
                                                if COURSE_DEMARREE:
                                                    Ind.Tdep[k] = TS_DEPART
                                                else:
                                                    Ind.Tdep[k] = Mes.ts_mesure[k]
                        ...
                        nb_arrive = 0
                        for k in range(0, nb_id):
                            if Ind.arrive[k] and Ind.donnee_precise[k]:  # and mode_course[k]: # on conserve le classement du cheval qui est arrivé
                        ...
                        if 1:
                            chevaux_en_course = np.where(
                                mode_course + Ind.en_course + Ind.donnee_precise == 3)  # seulement les chevaux entre le
                        ...
                        for k in range(0, nb_id):
                            ...
                            if Ind.donnee_precise[k]:
                                ...
                                Ind.pos_cheval_ind["long"] = 0.0
                                Ind.pos_cheval_ind["lat"] = 0.0
                                dict_indicateur["Pos"]["lat"] = Ind.pos_cheval_ind["lat"]
                                dict_indicateur["Pos"]["long"] = Ind.pos_cheval_ind["long"]
                        ...
                        if MODE == "course" and np.sum(Ind.en_course)>0:
                             with open('result_live.json', 'ab') as fp:
                    ...
                    try:
                        ...
                        msg_udp = "ErreurMoteur: impossible de fermer la socket#"
                        print msg_udp
                        logging.error(msg_udp, exc_info=True)
                    """
            else:
...
class Thread_B(threading.Thread):
    ...
    def run(self):
        ...
        global MODE # "attente" (0.1 Hz) ou "course" (10 Hz)
        global mode_course # tableau au nombre de partants. "CP" (course pure) "HC" (hors course) "CHC" (hors classement)
        ...
        try:
            ...
            ##### Message udp
            msg_udp = "EvtMoteur: parcours traite#"
            print msg_udp
            logging.info(msg_udp)
        except:
            logging.error(
