...
def recherche_prediction(Mes, k, trame, dist_totale):
    try:
        ...
        dir_x = Mes.x_mesure[k] - Mes.prev_x_mesure[k]
        dir_y = Mes.y_mesure[k] - Mes.prev_y_mesure[k]
        norm_dir = np.sqrt(np.square(dir_x) + np.square(dir_y))
...
class Parametres:
    ...
    def __init__(self):
        ...
        self.max_cov_etat = np.array([2,2,0.5]) * self.dt # np.array([0.4, 0.4, 0.2]) * self.dt #
        self.min_cov_etat = np.array([0.3,0.3,0.1]) * self.dt # self.max_cov_etat  #
        ...
        while True:
            try:
                ...
                while connected:
                    time.sleep(0.1)
                    try:
                        ##### TEST ##### data, addr = client_udp.recvfrom(100000)
                        if num_msg == 0:
                            data = "PathParcours:D:/Lisa/Documents/PMU/parcours/#"
                        if num_msg == 1:
                            data = "PathCourseParcours:D:/Lisa/Documents/PMU/parcours/#"
                        if num_msg == 2:
                            liste_id = [16, 32, 33, 37, 31,
                                        40]  # [11,12,13,14,18,21,22,29,39] #[13,18,21,22,23,26,29,34,35] # [16,19,20,31,32,33,37,40] #### TEST
                            print "liste id"
                            print liste_id
                                ...
                                try:
                                    ...
                                    if msg.startswith("PathParcours:"):
                                        ...
                                        try:
                                            ...
                                            liste_id = [16,32,33,37,31,40] #[11,12,13,14,18,21,22,29,39] #[13,18,21,22,23,26,29,34,35] # [16,19,20,31,32,33,37,40] #### TEST
                                            print "liste id"
                                            print liste_id
                                            nb_id = np.size(liste_id)
                                            etats_partants = np.zeros(nb_id)
                        ...
                        if 0: ####### TEST ##### #e.errno == 10054:
                            ...
                            logging.error("erreur udp", exc_info=True)
                            print "erreur udp"
            except:
