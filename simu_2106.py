...
def traitement_parcours(path_parcours, params,hote, port_sender, sender_udp):
    ...
    try:
        ...
        if np.sum(check_alignement) > 0:
            msg_udp = "WarningParcours: alignement interieur / exterieur, " + str(np.sum(check_alignement)) + " points sont imprecis#"
            print msg_udp
            logging.info(msg_udp)

        if np.sum(check_sens) > 0:
            msg_udp = "WarningParcours: avancement corde interieure, " + str(np.sum(check_sens)) + " points vont dans le mauvais sens#"
            print msg_udp
            logging.info(msg_udp)

        if np.sum(check_sens_int_ext) > 0:
            msg_udp = "WarningParcours: corde interieure / exterieure vont dans les sens opposes " + str(np.sum(check_sens_int_ext)) + " fois#"
            print msg_udp
            logging.info(msg_udp)
...
def init_depart(Mes, Ind, par, nb_handicaps, id_porte_dep, k, top_depart):
    ...
    if Mes.ind_parcours_mesure[k] > 0:
        ...
        if Mes.ind_parcours_mesure[k] > ind_depart:
            ...
            Ind.dist_parcourue_reelle_ind[k] = Mes.avance_mesure[k]
            Ind.dist_parcourue_proj_ind[k] = Mes.avance_mesure[k]
    print "indices ", k, Mes.ind_parcours_brut[k], Mes.ind_parcours_mesure[k]
...
def check_point_parcours(Mes, k, id_porte_dep):
    ...
    if prev_x != 0 and prev_y != 0 and x != 0 and y != 0:
        ...
        largeur_corde = np.sqrt(np.square(x_corde - x_corde_ext) + np.square(y_corde - y_corde_ext))
        ...
        if dist_corde_ext <= largeur_corde and dist_corde <= largeur_corde and \
                                dist_corde_ext + dist_corde < largeur_corde + 1:
            dans_parcours = True
...
def check_course_capteur(Mes, trame, k, id_porte_dep, nb_handicaps):
    ...
    if Mes.count_parcours_ok[k] > par.seuil_points_parcours and not Mes.depart_franchis[k]:
        ...
        if Mes.ind_parcours_brut[k] > ind_depart:
            Mes.ts_passage_dep[k] = Mes.ts_mesure[k] - float(np.sum(dD_corde_2[int(ind_depart):int(Mes.ind_parcours_brut[k])]))\
...
def check_depart_20s(Mes, Ind, recul_parcours, nb_handicaps, id_porte_dep, k, top_depart, TS_DEPART, COURSE_DEMARREE):
...
def calcul_corde(Mes, k):
    ...
    if ind_parcours > Mes.ind_parcours_mesure[k]:
        ...
        print "recul état ", k, Mes.ind_parcours_mesure[k], ind_parcours
...
def format_sortie_GPS(k, Mes, Ind):
    ...
    dict_indicateur = {"Capt": [], "Tmes": [], "Pos": {"lat": [], "long": []},
                       ...
                       "DmCo": [],
                       "HedA": [], "HedR": [], "Qmod": [], "Tdep": []}
    dict_indicateur["Tmes"] = Ind.ts_ind[k]
                    ...
                    while t_analyse < ts_fin and STATE_UDP and connected:
                        ...
                        for k in range(0, nb_id):
                            if etats_capteurs[k] > 0:
                                ...
                                if calculer_position and MODE == "course" and etats_capteurs[k] == 2 and not (Mes.arr_franchis[
                                    ...
                                    if (not COURSE_DEMARREE) or (COURSE_DEMARREE and not Mes.depart_franchis[k]):
                                        Mes, depart_parcours, recul_parcours = check_course_capteur(Mes, trame, k, id_porte_dep, nb_handicaps)
                                    ...
                                    Mes, Ind, TS_DEPART = check_depart_20s(Mes, Ind, recul_parcours, nb_handicaps,\
                                                                           id_porte_dep, k, top_depart, TS_DEPART, COURSE_DEMARREE)
                                    ...
                                    if Mes.depart_franchis[k] and not Ind.arrive[k]:
                                        ...
                                        Mat_etat, cov_obs, cov_etat, prev_etat, prev_P = recherche_prediction(Mes,k, trame)
                                        ...
                                        #### Sortie mesure
                                        Mes.lat_mesure[k] = m_to_lat(Mes.Etat[0], centre_lat) + centre_lat # trame[2] #
                                        Mes.long_mesure[k] = m_to_lon(Mes.Etat[1], centre_lat) + centre_long # trame[3] #
                        ...
                        if 1:
                            chevaux_en_course = np.where(
                                etats_partants + etats_capteurs + Ind.en_course + Ind.donnee_precise == 5)  # seulement les chevaux entre le
                        ...
                        for k in range(0, nb_id):
                            ...
                            elif etats_capteurs[k] == 1:
                                dict_indicateur = format_sortie_GPS(k, Mes, Ind)
                            elif etats_capteurs[k] == 2 and Ind.en_course[k] and Ind.donnee_precise[k]:
                                dict_indicateur = format_sortie_full(k, Mes, Ind)
                            elif etats_capteurs[k] == 2 and not Ind.en_course[k]:
                                dict_indicateur = format_sortie_GPS(k, Mes, Ind)
                            elif etats_capteurs[k] == 2 and Ind.en_course[k] and not Ind.donnee_precise[k]:
                                dict_indicateur = format_sortie_GPS(k, Mes, Ind)
                        ...
                        if 1: # MODE == "course" and not tous_arrives: #np.sum(Ind.en_course) > 0:
