...
def recherche_prediction(Mes, k, trame, dist_totale):
    try:
        ...
        decordage = float(Mes.stock_distance_corde_brut[k,-1] - \
                                                           Mes.stock_distance_corde_brut[k,-2])

        if abs(decordage) <= par.decordage_max and Mes.Obs[2]>0 and Mes.Obs[1]>0:
            dist_corde_cible = Mes.distance_corde_mesure[k] + decordage
        else:
            dist_corde_cible = Mes.distance_corde_mesure[k]
            if abs(decordage) > par.decordage_max:
                print 'decordage ', str(liste_id[k]), Mes.stock_distance_corde_brut[k,:]
        ...
        while avance < dD and ind_avance < np.size(corde_interpol_x):
            ...
            nouveau_x = corde_interpol_x[ind_avance] + vect_eq_x * dist_corde_cible # Mes.stock_distance_corde[k]
            # + vect_eq_x # update 0305
            nouveau_y = corde_interpol_y[ind_avance] + vect_eq_y * dist_corde_cible # Mes.stock_distance_corde[k]
...
class Parametres:
    ...
    def __init__(self):
        ...
        self.max_cov_etat = np.array([0.4, 0.4, 0.2]) * self.dt # np.array([2,2,0.5]) * self.dt
        self.min_cov_etat = self.max_cov_etat  #np.array([0.3,0.3,0.1]) * self.dt
        self.duree_dec_cov_etat = 30 * self.Hz
        self.sigma_dec_cov_etat = self.duree_dec_cov_etat / 2

        self.max_cov_v = 0.1 / np.float(3.6)
        self.min_cov_v = 0.1 / np.float(3.6) #0.03 / np.float(3.6)
        self.max_cov_mode = [100, 3, 1.5, 1, 0.03, 0.5] #[100, 5, 2, 1, 0.03, 0.5]
        self.min_cov_mode = self.max_cov_mode #[100, 2, 0.8, 1, 0.03, 0.5]
        self.duree_dec_cov = 5 * self.Hz # n secondes
        ...
        self.decordage_max = 0.3 # 1 mètre
...
class Moteur:
    ...
    def boucle_traitement(self):
        ...
        if DEPART_COURSE:
            COURSE_DEMARREE = 1
            self.tous_arrives = 0
            # self.TS_DEPART = np.max(Mes.ts_mesure)
            # if self.TS_DEPART == 0:
            #    d = datetime.datetime.utcnow()
            #    epoch = datetime.datetime(1970, 1, 1)
            #    self.TS_DEPART = (d - epoch).total_seconds()
            ...
            if etats_capteurs[k] > 0:
                ...
                if calculer_position and MODE == "course" and etats_capteurs[k] == 2 and not (
                    ...
                    if self.Mes.depart_franchis[k] and (not self.Ind.arrive[k]) and (not first_dep):
                        ...
                        Mat_etat, cov_obs, cov_etat, prev_etat, prev_P = recherche_prediction(self.Mes, k, trame, dist_param)
                    ...
                    if calculer_position and MODE == "course" and not self.tous_arrives:
                        if self.Ind.en_course[k]:
                            ...
                            vect_corde_x = corde_interpol_x[int(
                                min(self.Mes.ind_parcours_mesure[k], np.size(corde_interpol_x) - 1))] - corde_interpol_x[
                                               int(min(self.Mes.prev_ind_parcours_mesure[k],
                                                       np.size(corde_interpol_x) - 1))]
                            vect_corde_y = corde_interpol_y[int(
                                min(self.Mes.ind_parcours_mesure[k], np.size(corde_interpol_y) - 1))] - corde_interpol_y[
                                               int(min(self.Mes.prev_ind_parcours_mesure[k],
                                                       np.size(corde_interpol_y) - 1))]
                            vect_cheval_x = self.Mes.x_mesure[k] - self.Mes.prev_x_mesure[k]
                            vect_cheval_y = self.Mes.y_mesure[k] - self.Mes.prev_y_mesure[k]
                            ...
                            self.Ind.stock_heading_rel_1s[k, :] = np.append(self.Ind.stock_heading_rel_1s[k, 1:],
                                                                       heading_rel_inst)
                            self.Ind.heading_rel_ind[k] = np.mean(self.Ind.stock_heading_rel_1s[k, :])
        ...
        for k in range(0, nb_id):
            if self.Ind.arrive[k] and etats_partants[k] and self.Ind.donnee_precise[
                k]:  # and mode_course[k]: # on conserve le classement du cheval qui est arrivé
        ...
        if 1:
            chevaux_en_course = np.where(
                etats_partants + etats_capteurs + self.Ind.en_course + self.Ind.donnee_precise == 5)  # seulement les chevaux entre le
        ...
        for k in range(0, nb_id):
            ...
            if etats_capteurs[k] == 0:
                ...
                dict_indicateur = format_sortie_rien(k)
            elif etats_capteurs[k] == 2 and self.Ind.en_course[k] and self.Ind.donnee_precise[k] and etats_partants[k]:
...
class Thread_Reception_TCP(threading.Thread):
        ...
        while True:
            if STATE_UDP:
                ...
                try:
                    ...
                    while STATE_UDP and tcp_connected:
                        connexion_avec_serveur.settimeout(0.001)
                        try:
                        ...
                        for i in range(0, len(msg_recu)):
                            ...
                            if np.size(frame) >= par.size_trame:
                                if np.size(frames.content) == 0:
                                    frames.content = frame
                                else:
                                    frames.content = np.vstack([frames.content, frame])
                                frame = []
                            ...
                            for i in range(0, len(msg_recu)):
                                ...
                                if np.size(frame) >= 7:
                                    if np.size(frames.content) == 0:
                                        ...
                                        frames.content = np.vstack([frames.content, frame])
                                    frame = []
                        if frames.content.__len__() > 100:
                            frames.content = frames.content[1:-1][:]
                        frames.lock.release()
                        # sauvegarde datas brutes

                except Exception as e:
                    msg_udp = "ErreurMoteur: erreur thread reception TCP#"
                    print msg_udp
                    logging.error(msg_udp, exc_info=True)
                    sender_udp.sendto(msg_udp, (hote, port_sender))
                    try:
                                ...
                                try:
                                    ...
                                    if msg.startswith("PathParcours:"):
                                        ...
                                        try:
                                            ...
                                            #liste_id = [11,12,13,14,18,21,22,29,39] #[16,32,33,37,31,40] #[13,18,21,22,23,26,29,34,35] # [16,19,20,31,32,33,37,40] #### TEST
                                            print "liste id"
                                            print liste_id
                                        ...
                                        try:
                                            ...
                                            for etat in liste_etats:
                                                if not etat == '':
                                                    ...
                                                    if partant in liste_id:
                                                        for k in range(0, np.size(liste_id)):
                                                            if partant == liste_id[k]:
                                                                indice_partant = k
...
def main():
    ...
    recep = Thread_Reception_TCP("Reception TCP")
