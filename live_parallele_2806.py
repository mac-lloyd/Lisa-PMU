...
def hhmmss_to_ts_utc(hhmmss):
	...
	t2 = datetime.datetime(1970, 1, 1,0,0,0,0)
	timestamp  = int((t - t2).total_seconds() * 1000)
	#timestamp = (time.mktime(t.timetuple()) - time.mktime(t2.timetuple())) *1000
...
class Parametres:
    ...
    def __init__(self):
        ...
        self.cov_mode_v = np.square(np.float(0.5) / np.float(3.6))
            ...
            if etats_capteurs[k] > 0:
                ...
                if etats_capteurs[k] == 2:
                    ...
                    if self.Ind.count_precision_2m[k] > 50:
                        msg_udp = "capteur " + str(k) + "sorti " + str(self.num_analyse) + " " + str(self.Mes.ts_mesure[k])
                        print msg_udp
