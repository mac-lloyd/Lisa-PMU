...
class Parametres:
    ...
    def __init__(self):
        ...
        self.cov_etat = np.square(np.array([1, 1, 0.5]) * self.dt)  # np.square(np.array([0.2, 0.2, 0.1]) * self.dt)
            ...
            if STATE_UDP:
                ...
                try:
                    ...
                    while STATE_UDP and connected:
                        ...
                        print msg_udp
                        logging.info(msg_udp)
                        msg_udp = "dt_thread ", dt_thread
                        print msg_udp
                        logging.info(msg_udp)
                        # réception du message TCP
                        connexion_avec_serveur.settimeout(max(0.00001, float(next_t - time.time())))
                        pause.until(next_t)
                        t = next_t
                        try:
                            msg_recu = connexion_avec_serveur.recv(100000)
                            connexion_avec_serveur.settimeout(None)
                        ...
                        # print(msg_recu)
                            ...
                            if e.errno == 10054:
                                ...
                                try:
                                    ...
                                    msg_udp = "impossible de fermer la socket"
                                ...
                                if Mes.num_trames_recues[k] == 0:
                                    ...
                                    if trame[2] != 0 and trame[3] != 0 and trame[1]!= 0 and hhmmss_to_ts_utc(trame[1]) > Mes.ts_mesure[k]:
                                        ...
                                        if Mes.ts_mesure[k] > 0:
                                            Mes.ts_mesure[k] = Mes.ts_mesure[k] + 100
                                        #if hhmmss_to_ts_utc(trame[1]) <= Mes.ts_mesure[k]:
                                        # print Mes.ts_mesure[k], hhmmss_to_ts_utc(trame[1]), k
                            ...
                            if calculer_position and MODE == "course" and not (Mes.arr_franchis[
                                ...
                                if Mes.depart_franchis[k] and not Ind.arrive[k]: # (Mes.arr_franchis[k] or ):
                                    if trame[2] != 0 and trame[3] != 0 and trame[1]!= 0 :
                                        if (trame[4] == 0.0 and Mes.v_mesure[k] > 3.0) or (trame[4] > 20.0) \
                                        ...
                                        if mindist > diagonale or mindist_ext > diagonale:
                                            Mes.Obs = [0, 0, 0]
                                            print "hors parcours", mindist, mindist_ext, diagonale , Mes.ts_mesure[k
                                            ], str(k)
                                        ...
                                        try:
                                            ...
                                            for etat in liste_etats:
                                                if not etat == '':
                                                    ...
                                                    mode_course[indice_partant] = not int(float(num_etat[1]))
